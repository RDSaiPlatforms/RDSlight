...
class ReflectiveDialogueSystem:
    ...
    def select_gpt_model(self):
        """
        Prompt the user to select a model for generating the final response.
        """
        print("\n" + Fore.GREEN + "Select the model to use for generating the final response:")
        print(Fore.YELLOW + "1. gemini-lite")
        print(Fore.YELLOW + "2. gemini-pro")
        print(Fore.YELLOW + "3. gemini-ultra")

        # Model selection
        while True:
            model_choice = input(Fore.WHITE + "Enter the number corresponding to your choice (1, 2 or 3): ").strip()
            if model_choice == '1':
                print(Fore.GREEN + "\nModel selected: gemini-lite\n")
                return "gemini-lite"
            elif model_choice == '2':
                print(Fore.RED + "Warning: gemini-pro is more powerful and may cost more to use. Do you want to continue? (y/n)")
                confirm_choice = input(Fore.WHITE + "Enter 'y' to confirm or 'n' to select another model: ").strip().lower()
                if confirm_choice == 'y':
                    print(Fore.GREEN + "\nModel selected: gemini-pro\n")
                    return "gemini-pro"
            elif model_choice == '3':
                print(Fore.RED + "Warning: gemini-ultra is the most powerful and may cost the most. Do you want to continue? (y/n)")
                confirm_choice = input(Fore.WHITE + "Enter 'y' to confirm or 'n' to select another model: ").strip().lower()
                if confirm_choice == 'y':
                    print(Fore.GREEN + "\nModel selected: gemini-ultra\n")
                    return "gemini-ultra"
            else:
    ...
    def process_user_input(self, user_input):
        ...
        # Step 2: Detect category using Gemini API
    ...
    def generate_response_strategy(self, user_input, sentiment, internal_prompt, question_type):
        if question_type == 'technical':
            ...
            prompt = (
                ...
                f"how can I respond in a helpful and human-like way?"
            )
    ...
    def generate_final_response(self, user_input, response_strategy, internal_prompt, question_type):
        if question_type == 'technical':
            prompt = (
                ...
                f"my response should be highly accurate and efficient."
            )
        else:
            prompt = (
                f"This is what my response should look like, '{response_strategy}', and considering the internal reflection: '{internal_prompt}', "
                f"I should generate a response that is helpful and human-like."
            )
